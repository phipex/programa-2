...
class Regression {
    ...
    static sum(list) {
        ...
        var i, sumOne = 0, sumTwo = 0, meanOne = 0, meanTwo = 0, squareOne = 0, squareTwo = 0, product = 0; 
        for (i = 0; i < len; i++) {
            sumOne += list.getValueOne(i);
            sumTwo += list.getValueTwo(i);
            squareOne += Math.pow(list.getValueOne(i), 2);
            squareTwo += Math.pow(list.getValueTwo(i), 2);
            product += (list.getValueOne(i) * list.getValueTwo(i));
        }
